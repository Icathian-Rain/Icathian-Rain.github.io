...
在RESTful架构中，每个网址代表一种`资源（resource）`，所以网址中不能有动词，只能有名词，而且所用的名词往往与数据库的表格名对应。一般来说，数据库中的表都是同种记录的"集合"（collection），所以API中的名词也应该使用复数。
...
- PATCH（UPDATE）：在服务器更新资源（客户端提供`改变的属性`）。
